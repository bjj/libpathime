 ...
 * once backend.h exists — running each compiled-in backend's one-time global
 * initialization (anthy_init, pyzy's shared Database and SpecialPhraseTable)
 * through its lifetime hooks. It is documented as the one slow call, so eager
 * global work belongs here and nowhere else. libhangul needs nothing at this
 * layer in our build; the marked gap below says why, and corrects TODO.md §2
 * Finding 3 on the point.
 *
 * What is real today is everything that needs no backend: the process-global
 * flag behind pathime::initialized(), the resolved directory behind
 * pathime::data_dir(), and the full validation and already-initialized
 * contract. The backend hooks themselves are a single marked gap, because
 * backend.h is deliberately undesigned until the composition representation
 * settles (TODO.md §3, question 1).
 *
...
pathime_status_t pathime_init(const pathime_init_params_t *params)
{
    ...
    /*
     * TODO(impl): drive each compiled-in backend's one-time global init here,
     * through the lifetime hooks backend.h will declare — anthy_init()
     * (anthy-unicode/anthy/anthy.h:53) and
     * PyZy::InputContext::init(user_cache_dir, user_config_dir)
     * (pyzy/src/InputContext.h:399, the shared Database and
     * SpecialPhraseTable) — each gated on its PATHIME_WITH_* macro and each
     * pointed at `resolved` rather than at the location it would pick from the
     * environment itself. Creating the directory belongs with them: it is the
     * first thing they need and nothing above this line touches the
     * filesystem.
     *
     * libhangul is deliberately absent from that list, against what TODO.md §2
     * Finding 3 says. hangul_init() and hangul_fini() exist only under
     ...
     * off to avoid an EXPAT dependency and a sed-based codegen step. Without
     * it there is no keyboard registry to populate: the nine built-in layouts
     * are static tables, so the hangul backend has no process-global setup at
     * all in this build. Should external keyboards ever be turned on, the pair
     * belongs here and hangul_init() takes the user keyboard path — one more
     * thing that would be rooted at `resolved`.
     *
     * On a hook failure: run the shutdown counterparts of the hooks that
     * already succeeded, in reverse order, and return PATHIME_ERROR_BACKEND
     * with g_initialized still false, so the caller may simply retry.
     */
...
void pathime_shutdown(void)
{
    ...
    /*
     * TODO(impl): drive the per-backend global shutdown hooks here, in the
     * reverse of the order pathime_init() ran them —
     * PyZy::InputContext::finalize() (pyzy/src/InputContext.h:407, which is
     * also where pyzy's user database is written, since its g_timeout_add save
     * never fires without a GMainLoop: TODO.md §5), then anthy_quit()
     * (anthy-unicode/anthy/anthy.h:54). Nothing for hangul, for the reason
     * given at the matching point in pathime_init().
     *
