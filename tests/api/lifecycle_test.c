...
{
    /*
     * Initialized in the declaration, which is what the header asks of every
     * caller and what the cases below depend on. `struct_size == sizeof` tells
     * the library every member is filled in, so it reads all of them —
     * including resource_dir, which it dereferences to reject an empty one
     * (src/init.cc). The cases here vary struct_size and data_dir and never
     * mention resource_dir, so the initializer is the only thing standing
     * between this test and a wild pointer.
     */
