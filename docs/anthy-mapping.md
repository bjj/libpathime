...
| **Commit (forced)** | No single equivalent | anthy has no "end the composition" call, because it does not own the composition: the pre-conversion kana lives in the wrapper. So this routes on the same state Return does — mid-conversion it is the converted segments plus the unconverted tail, while typing it is the kana with a pending romaji consonant resolved. |
