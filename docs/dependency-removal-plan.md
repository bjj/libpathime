...
**Status: both work streams are implemented on this branch and verified on
Windows** (full ctest green, install ships zero external DLLs,
`examples/install-check` passes against the install). The pyzy commits are on
the submodule's local `libpathime` branch — **nothing is pushed anywhere**,
including this branch. Remaining: review; push `bjj/pyzy` then this branch;
let CI prove the Linux/macOS legs (unchanged in behaviour — POSIX still links
system sqlite and never linked the removed glib into our artifacts, but the
GlibLess code paths themselves are Windows-verified only).
...
## Work stream B — sqlite3 links statically on Windows (vcpkg static triplet)

Findings that shaped this:

- The API surface is 28 functions, but the engine features are broad (ATTACH,
  WAL, the backup API, `LIKE … ESCAPE`, `datetime()`, URI filenames), so a
  minimised custom build would buy ~100–150 KB at best. The stock build is
  fine; nothing SQLite-side needs configuring.
- **Two embedded copies** result (one in `pathime.dll`, one in
  `pyzy-1.0.dll`), ~+1 MB net against shipping the DLL — measured. Benign on
  Windows: separate symbol spaces, no interposition, and the two copies never
  open the same file (pyzy's databases live under `…/pyzy/`, the table
  engine's under `…/table/`). On Linux the same move would be hazardous —
  default-visibility ELF exports would unify `sqlite3_*` across the two
  shared objects and could hijack a consumer's own sqlite — so **POSIX stays
  on the system sqlite**, which also matches the artifact policy (POSIX
  ships no external libraries).
- **How the static build arrives matters.** Committing the amalgamation to
  this repository is unacceptable (a 9.5 MB third-party source permanently
  in history for two files); a submodule holding it is workable but means
  hosting another repository. vcpkg's `x64-windows-static-md` triplet
  delivers a pinned static SQLite through the same `find_package(SQLite3)`
  the build already runs — vcpkg installs a find-module wrapper that
  selects the debug or release archive per configuration — so the whole
  change is the triplet selection plus documentation.
...
- CI/release Windows vcpkg installs become
  ...
  pkg-config consumer checks run on Linux).
- Docs: `BUILD.md` (Windows prerequisites), `THIRD-PARTY.md` (no
  `sqlite3.dll` row; SQLite is static inside the Windows libraries),
  `docs/windows-port.md`, `docs/design-history.md`,
  `cmake/LibpathimeDependencies.cmake` hint strings.
- Nothing else: `find_package(SQLite3)`, the consumer config, and the
  pkg-config `Requires.private sqlite3` entry are all unchanged — a static
  Windows consumer resolves SQLite from the same vcpkg triplet.

## Sequencing and verification

1. Work stream A, then B. After B the Windows install ships zero external
   DLLs.
